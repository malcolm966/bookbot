...
def get_csv_status(status, data):
    def handle_pending(d):
        # list[list[Any]] -> list[list[str]]
        return list(
            map(
    ...
    def handle_processing(d):
        # list[list[str]] -> CSV string
        rows = map(lambda row: ",".join(row), d)
        return "\n".join(rows)

    match status:
        case CSVExportStatus.PENDING:
            converted = handle_pending(data)
            return "Pending...", converted

        case CSVExportStatus.PROCESSING:
            csv_string = handle_processing(data)
        ...
        case CSVExportStatus.FAILURE:
            converted = handle_pending(data)
            csv_string = handle_processing(converted)
